...
CStereoVisionDlg::CStereoVisionDlg(CWnd* pParent /*=NULL*/)
  : CDialog(CStereoVisionDlg::IDD, pParent)
    ...
    , m_bModeHH(FALSE)
    , m_bSaveFrame(FALSE)
    , m_dAlpha(0) { m_hIcon = AfxGetApp()->LoadIcon(IDR_MAINFRAME); }
...
BOOL CStereoVisionDlg::OnInitDialog() {
  ...
  for (int i = 0; i < m_nCamCount; i++) {
    ...
    } else AfxMessageBox(_T("不能获取摄像头的名称"));
  }
  camstr.ReleaseBuffer();
  if (m_nCamCount <= 1) m_CBNCamList_R.EnableWindow(FALSE);
...
//显示。
HCURSOR CStereoVisionDlg::OnQueryDragIcon() { return static_cast<HCURSOR>(m_hIcon); }
...
void CStereoVisionDlg::OnBnClkRunCam() {
  ...
  if (m_nCamCount > 0) {
    if (m_nImageWidth / m_nImageHeight >= 2) { lfCam.init(0, m_stereoImageBuffer); } else {
      lfCam.init(3, m_stereoImageBuffer);
    }
    //打开第一个摄像头
    ...
    lfCam.m_capture.set(CV_CAP_PROP_FRAME_WIDTH, m_nImageWidth);
    lfCam.m_capture.set(CV_CAP_PROP_FRAME_HEIGHT, m_nImageHeight);
  }
  if (m_nCamCount > 1) {
    ...
    if (m_nImageWidth / m_nImageHeight >= 2) {
      riCam.init(1, m_stereoImageBuffer);
      if (!riCam.open(-1)) {
      ...
      riCam.init(3, m_stereoImageBuffer);
      //打开第二个摄像头
    //if( ! riCam.OpenCamera(m_lfCamID, false, m_nImageWidth, m_nImageHeight) ) 
      ...
      riCam.m_capture.set(CV_CAP_PROP_FRAME_WIDTH, m_nImageWidth);
      riCam.m_capture.set(CV_CAP_PROP_FRAME_HEIGHT, m_nImageHeight);
    }    
  }
...
void CStereoVisionDlg::OnTimer(UINT_PTR nIDEvent) {
  ...
  if (lfCam.isOpened()) {
    ...
    if (m_ProcMethod != SHOW_ORIGINAL_FRAME) { DoFrameProc(lfFrame, lfFrame); }
  ...
  if (riCam.isOpened()) {
    ...
    if (m_ProcMethod != SHOW_ORIGINAL_FRAME) { DoFrameProc(riFrame, riFrame); }
...
bool CStereoVisionDlg::DoParseOptionsOfCameraCalib(OptionCameraCalib& opt) {
  ...
  UINT nCheckIDs[9] = {
    IDC_CHK1FPP, IDC_CHK2UIG, IDC_CHK3FAR, IDC_CHK4ZTD,
  ...
  opt.calibOrder = m_nID_RAD == IDC_RAD_Load1CamCalibResult
                     ? CALIB_LOAD_CAMERA_PARAMS
                     : m_nID_RAD == IDC_RAD_Calib1CamFirst
                     ? CALIB_SINGLE_CAMERA_FIRST
                     : m_nID_RAD == IDC_RAD_StereoCalib
  ...
  // 确定双目校正缩放系数
  if (m_dAlpha < 0 || m_dAlpha > 1) m_dAlpha = -1;
  opt.alpha = m_dAlpha;
  ...
  for (i = 0; i < 4; i++) {
    ...
    if (m_pCheck->GetCheck()) opt.flagCameraCalib |= nFlagDefs1[i];
  }
  for (i = 4; i < 9; i++) {
    m_pCheck = (CButton*)GetDlgItem(nCheckIDs[i]);
    if (m_pCheck->GetCheck()) opt.flagStereoCalib |= nFlagDefs2[i - 4];
  }
...
void CStereoVisionDlg::OnBnClk_DoCameraCalib() {
  ...
  if (!DoParseOptionsOfCameraCalib(optCalib)) return;
  ...
  try {
    ...
    if (optCalib.loadConerDatas) {
      if (m_stereoCalibrator.loadCornerData(cornerFile, cornerDatas)) { AfxMessageBox(_T("已读入角点坐标数据")); } else {
      ...
      ////////////////////////////////////////////////////////////////////////
      // 初始化角点数据集
      m_stereoCalibrator.initCornerData(optCalib.numberBoards, imageSize, optCalib.cornerSize, optCalib.squareSize,
                                        cornerDatas);
      ...
      while (nFoundBoard < optCalib.numberBoards) {
        ...
        {
          lfCam >> frame0;
          if (optCalib.doStereoCalib) riCam >> frame1;
        }
        if (frame0.empty()) break;
        if (optCalib.doStereoCalib) { if (frame1.empty()) break; }
    ...
    if (optCalib.doStereoCalib) // 双目标定和校正
    {
      ...
      m_stereoCalibrator.saveCalibrationDatas(filePath.GetBuffer(0), optCalib.rectifyMethod, cornerDatas, stereoParams,
                                              remapMatrixs);
    ...
    } else // 单目标定
    {
      ...
      // 计算标定误差
      double avgErr = 0;
      m_stereoCalibrator.getCameraCalibrateError(cornerDatas.objectPoints, cornerDatas.imagePoints1, cameraParams,
                                                 avgErr);
      char info[50];
    ...
    // 显示标定效果
    {
      ...
      while (true) {
        ...
        if (showCalibrated) m_stereoCalibrator.remapImage(img0, img1, img0, img1, remapMatrixs);
    ...
    //////////////////////////////////////////////////////////////////////////
    // 错误信息处理
  catch (LPCTSTR errMsg) { AfxMessageBox(errMsg); }
...
bool CStereoVisionDlg::DoParseOptionsOfStereoMatch(OptionStereoMatch& opt) {
  ...
  opt.stereoMethod = m_nID_RAD == IDC_RAD_BM
                       ? STEREO_BM
                       : m_nID_RAD == IDC_RAD_SGBM
                       ? STEREO_SGBM
                       : m_nID_RAD == IDC_RAD_VAR
                       ? STEREO_VAR
                       : STEREO_BM;
  // 确认左右视图的来源（摄像头 or 本地图片）
...
void CStereoVisionDlg::OnBnClk_DoCompDisp() {
  ...
  if (!DoParseOptionsOfStereoMatch(optMatch)) return;
  ...
  try {
    ...
    } else m_stereoMatcher.init(img1.cols, img1.rows, "");
    ...
    while (true) {
      ...
      // 载入图像
      if (optMatch.readLocalImage) {
        if (frameCount >= MIN(imgFiles1.size(), imgFiles2.size())) break;
      ...
      if (optMatch.generatePointCloud) {
        ...
        if (!objectInfos.empty() && (clock() - tShowObjInfo) > 500) {
          ...
          m_ObjectHeight = objectInfos[0].boundRect.height * objectInfos[0].distance / fl;
          m_ObjectHeight = (int)(m_ObjectHeight * 10000) / 10000.;
          m_ObjectWidth = objectInfos[0].boundRect.width * objectInfos[0].distance / fl;
      ...
      // 保存画面和视差图像
      if (optMatch.saveResults) F_Saveframe(img1, img2, disp);

      if (optMatch.delayEachFrame) Sleep(m_nDelayTime * 1000);
    }
...
void CStereoVisionDlg::OnBnClkDefaultStereoParam() {
  ...
  if (m_nID_RAD == IDC_RAD_BM) {
    ...
    m_stereoMatcher.m_VAR.flags = m_stereoMatcher.m_VAR.USE_SMART_ID | m_stereoMatcher.m_VAR.USE_AUTO_PARAMS |
      m_stereoMatcher.m_VAR.USE_INITIAL_DISPARITY | m_stereoMatcher.m_VAR.USE_MEDIAN_FILTERING;
  }
  UpdateData(FALSE);
}
...
void CStereoVisionDlg::OnDeltaposSpin_MaxDisp(NMHDR* pNMHDR, LRESULT* pResult) {
  ...
  *pResult = 0;
  ...
  if (m_nNumDisp < 16) m_nNumDisp = 16;
  if (m_nNumDisp > 256) m_nNumDisp = 256;
...
void CStereoVisionDlg::OnDeltaposSpin_SADWinSiz(NMHDR* pNMHDR, LRESULT* pResult) {
  ...
  *pResult = 0;
  ...
  if (m_nID_RAD == IDC_RAD_BM) {
    ...
    if (m_nSADWinSiz % 2 == 0) m_nSADWinSiz += 1;
    if (m_nSADWinSiz < 5) m_nSADWinSiz = 5;
    if (m_nSADWinSiz > 21) m_nSADWinSiz = 21;
    UpdateData(FALSE);
  } else if (m_nID_RAD == IDC_RAD_SGBM) {
    UpdateData(TRUE);
    m_nSADWinSiz += (int)pNMUpDown->iDelta * 2;
    if (m_nSADWinSiz % 2 == 0) m_nSADWinSiz += 1;
    if (m_nSADWinSiz < 1) m_nSADWinSiz = 1;
    if (m_nSADWinSiz > 11) m_nSADWinSiz = 11;
    UpdateData(FALSE);
  }
}
...
void CStereoVisionDlg::OnDeltaposSpin_SpeckRange(NMHDR* pNMHDR, LRESULT* pResult) {
  ...
  *pResult = 0;
  ...
  if (m_nID_RAD == IDC_RAD_BM) {
    ...
    if (m_nSpeckRange < 0) m_nSpeckRange = 0;
    if (m_nSpeckRange > 64) m_nSpeckRange = 64;
    UpdateData(FALSE);
  } else if (m_nID_RAD == IDC_RAD_SGBM) {
    UpdateData(TRUE);
...
void CStereoVisionDlg::OnDeltaposSpin_ViewWidth(NMHDR* pNMHDR, LRESULT* pResult) {
  ...
  *pResult = 0;
  ...
  if (m_nViewWidth < 60) m_nViewWidth = 60;
  if (m_nViewWidth > 600) m_nViewWidth = 600;
...
void CStereoVisionDlg::OnDeltaposSpin_ViewHeight(NMHDR* pNMHDR, LRESULT* pResult) {
  ...
  *pResult = 0;
  ...
  if (m_nViewHeight < 40) m_nViewHeight = 40;
  if (m_nViewHeight > 400) m_nViewHeight = 400;
...
void CStereoVisionDlg::OnDeltaposSpin_ViewDepth(NMHDR* pNMHDR, LRESULT* pResult) {
  ...
  *pResult = 0;
  ...
  if (m_nViewDepth < 60) m_nViewDepth = 60;
  if (m_nViewDepth > 600) m_nViewDepth = 600;
...
bool CStereoVisionDlg::F_CheckDir(const string dir, bool creatDir /* = false */) {
  ...
  if (Handle == -1L) { if (creatDir) { if (_mkdir(dir.c_str()) == 0) { RetVal = true; } } } else {
    //RetVal = true;
    //_findclose(Handle);
  }
...
void CStereoVisionDlg::F_EdgeDetectCanny(Mat& src, Mat& des) {
  ...
  else cvtColor(src, gray, CV_RGB2GRAY);
  ...
  // 将边缘检测图像转至输出图像格式
  if (des.channels() == 1) edge8u.copyTo(des);
  else cvtColor(edge8u, des, CV_GRAY2BGR);
}
...
void CStereoVisionDlg::DoFrameProc(Mat& src, Mat& dst) {
  try {
    switch (m_ProcMethod) {
    case SHOW_EDGE_IMAGE: { F_EdgeDetectCanny(src, dst); }
    break;
    case SHOW_EQUAL_HISTOGRAM: {
      ...
      split(src, rgb);
      // 对各个分量依次进行直方图均衡
      for (int i = 0; i < 3; i++) { equalizeHist(rgb[i], rgb[i]); }
      // 合并各分量
      merge(rgb, dst);
    }
    break;
    case SHOW_CONVERT_COLOR: {
      ...
      int type =
        m_nID_RAD == IDC_RAD_ToHSV
          ? CV_BGR2HSV
          : m_nID_RAD == IDC_RAD_ToHLS
          ? CV_BGR2HLS
          : m_nID_RAD == IDC_RAD_ToLab
          ? CV_BGR2Lab
          : m_nID_RAD == IDC_RAD_ToLuv
          ? CV_BGR2Luv
          : m_nID_RAD == IDC_RAD_ToYCrCb
          ? CV_BGR2YCrCb
          : CV_BGR2XYZ;
